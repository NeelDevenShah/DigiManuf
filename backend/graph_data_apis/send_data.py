...
def fetch_anomaly_data_for_sensors_by_all_category(organization_id: str, unit_id:str, machine_id: str, start_time: datetime = None, end_time: datetime = None):
    client = CosmosClient(COSMOS_DB_ENDPOINT, COSMOS_DB_KEY)
    database = client.get_database_client(DATABASE_NAME)
    container = database.get_container_client(CONTAINER_NAME)
...
def fetch_sensor_ids_by_category(organization_id: str, unit_id: str, machine_id: str, sensor_category: str):
    conn = pyodbc.connect(SQL_CONNECTION_STRING)
    cursor = conn.cursor()
    ...
    SELECT sensor_id
    FROM sensor_categories
    WHERE organization_id = ? AND unit_id = ? AND machine_id = ? AND category = ?
    """
    
    cursor.execute(query, (organization_id, unit_id, machine_id, sensor_category))
...
# TODO Changes
def fetch_anomaly_data_for_sensors_by_category(organization_id: str, unit_id:str, machine_id: str, sensor_category: str, start_time: datetime = None, end_time: datetime = None):
    client = CosmosClient(COSMOS_DB_ENDPOINT, COSMOS_DB_KEY)
    database = client.get_database_client(DATABASE_NAME)
    container = database.get_container_client(CONTAINER_NAME)
